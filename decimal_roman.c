...
// storing the pre_digit_characters for example: 40 has to be stored as XL
...
/*while loop to return the roman string of the passed decimal number. Inside the loop it calls either post_digit_characters 
or pre_digit_characters depending on the input */
	while(decimal_number !=0)
	{
		if(decimal_number >= 1000){ // converstion for 1000
			buffer = post_digit_characters('M',decimal_number/1000,buffer);
			...
			if(decimal_number <(500 + 4*100 )){
				buffer = post_digit_characters('D',decimal_number/500,buffer);
				decimal_number = decimal_number - (decimal_number/500) * 500;
			}
			else{
				buffer = pre_digit_characters('C','M',buffer);
			...
			if(decimal_number < (100 + 3 * 100)){
				buffer = post_digit_characters('C',decimal_number/100,buffer);
				decimal_number = decimal_number - (decimal_number/100) * 100;
			}
			else{
				buffer = pre_digit_characters('L','D',buffer);
			...
			if(decimal_number < (50 + 4 * 10)){
				buffer = post_digit_characters('L',decimal_number/50,buffer);
				decimal_number = decimal_number - (decimal_number/50) * 50;
			}
			else{
				buffer = pre_digit_characters('X','C',buffer);
			...
			if(decimal_number < (10 + 3 * 10)){
				buffer = post_digit_characters('X',decimal_number/10,buffer);
				decimal_number = decimal_number - (decimal_number/10) * 10;
			}
			else{
				buffer = pre_digit_characters('X','L',buffer);
			...
			if(decimal_number < (5 + 4 * 1)){
				buffer = post_digit_characters('V',decimal_number/5,buffer);
				decimal_number = decimal_number - (decimal_number/5) * 5;
			}
			else{
				buffer = pre_digit_characters('I','X',buffer);
			...
			if(decimal_number < 4)
			{
				buffer = post_digit_characters('I',decimal_number/1,buffer);
				decimal_number = decimal_number - (decimal_number/1) * 1;
			}
			else{
				buffer = pre_digit_characters('I','V',buffer);
